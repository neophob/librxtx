		...
		jboolean interrupted )
{
	...
	*/
	...
	//some JNI changed I made... cant remember why...
	//body = (*env)->GetByteArrayElements( env, jbarray, 0 );
	body = (*env)->GetPrimitiveArrayCritical( env, jbarray, 0 );
...
#endif */ /* __sun__ */
//	(*env)->ReleaseByteArrayElements( env, jbarray, body, 0 );
	(*env)->ReleasePrimitiveArrayCritical( env, jbarray, body, 0 );
...
int fhs_lock( const char *filename, int pid )
{
	...
	//fix http://bugzilla.qbang.org/show_bug.cgi?id=160
